...
class Snake:
    ...
    # Constants
    MIN_DISTANCE_BETWEEN_POINTS = 1    # The minimum distance between two points to consider them overlaped
    ...
    # Constructor
    #################################
    def __init__( self, image = None, closed = True , contour = []):
        """
        Object constructor
        ...
        self.image = (image/ image.max())*255
        self.image = np.float32(np.stack([self.image,self.image,self.image],axis=-1))
        ...
        self.gray = cv2.cvtColor( self.image, cv2.COLOR_RGB2GRAY ).astype('uint8')
        ...
        # cv2.imwrite("tests/mrimg_gray.png", self.gray)
        # cv2.imwrite("tests/mrimg_binary.png", self.binary)
        # cv2.imwrite("tests/mrimg_gradientX.png", self.gradientX)
        # cv2.imwrite("tests/mrimg_gradientY.png", self.gradientY)
